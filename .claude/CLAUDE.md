...
- **The page owns the device safe areas.** `layout.tsx` exports `viewport: { viewportFit: "cover" }`, so iOS stops insetting the page out of the notch and home-indicator strips: a `fixed; inset: 0` overlay reaches the screen edges, which a modal backdrop needs. The cost is that anything pinned to a viewport edge must pad itself back out, via `--safe-top` / `--safe-bottom` in `globals.css`. Today that is `Nav`, `StatsPanel`, `FilterSheet`, `ModalShell` and the homepage tiles.
