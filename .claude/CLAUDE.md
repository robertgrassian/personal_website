...
- **Comments are graded for length at commit time.** `.claude/hooks/comment-brevity.sh` is a
  `PreToolUse(Bash)` hook wired up in `.claude/settings.json`: on any command containing
  `git commit` it collects the comment lines the commit would add (source files only, never
  Markdown) and asks Haiku whether any are padded. If so it blocks the commit once and prints
  what to cut. It fires **at most once per distinct set of comments**, so disagreeing with it
  means committing the same text again rather than arguing. It fails open on any error, and
  stays silent when it passes. `MIN_CHARS` in that script sets the floor below which no model
  call happens.
