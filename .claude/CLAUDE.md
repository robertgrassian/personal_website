...
- **Never touch `TODO.md` without invoking the `todo` skill first.** It owns that file: adding, completing, reordering, rewording, answering "what's next", or any direct edit. This applies to conversational phrasings ("add that to my todos", "what should I work on") exactly as much as to a typed `/todo`. The structure rules live in the skill, deliberately not restated here — when they were duplicated into this file, having them already in context is precisely what made the skill feel redundant and got it skipped for a whole session.
