...
**Written 2026-07 while choosing the database**, to answer: _"Supabase is described as
backend-as-a-service — is that how we'd use it? And if so, why have a Python backend at all
...
The **"backend-as-a-service" pitch** is what you get when you use _all_ of it: the browser
...
### Mode B — Supabase as infrastructure only (what the site shipped)
...
If the Python requirement were dropped, the _objectively_ simplest architecture is no
...
## 3. So what _is_ the point of the Python backend?
...
- The **IGDB proxy** (secret credentials, token caching, rate limiting) — needs a server in
  _every_ mode; the only question is whether that server is FastAPI, an Edge Function, or a
  Next route handler.
...
- Mode A couples the _entire app_ to Supabase: every FE data call is `supabase-js`,
...
so the backend remains a place where deep existing expertise applies while the _frontend_ is
...
| _Unfamiliar_-parts count for this developer | High (RLS, PostgREST semantics, Deno)  | **Low** (FastAPI ≈ Spring-shaped)               | Medium (all logic in TS/Next) |
...
optimizes _conceptual_ simplicity for a backend engineer by keeping the architecture
...
Python runtime disappointed on Vercel. It did not: the spike measured ~80ms function
responses, and the serverless Python backend has run in production since.

## 5. Exactly which Supabase slices the site uses
...
| Storage                     | ❌ (revisit)       | Candidate home for mirrored cover art if IGDB hotlinking becomes a concern; also the likely home for profile pictures (see `TODO.md`)                                                                                                         |
| Edge Functions              | ❌                 | FastAPI is our function runtime                                                                                                                                                                                                               |

**Net:** we are _not_ using Supabase as a BaaS. We're using it as a Postgres host that
